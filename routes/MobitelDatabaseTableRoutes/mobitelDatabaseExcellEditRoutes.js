...
router.route('/mobitelProjectsDatabasesExcell/ExcellEdit').put(async (req, res) => {
  const PostsArray = req.body;

  try {
  
    await PostsArray.forEach(async post => {
      // console.log(post);
      ...
      let NotNullObject = Object.fromEntries(Object.entries(updatePost).filter(([_, v]) => v != null));
      // console.log(NotNullObject);

      await Posts.updateOne(
        { "Planning_ID": post.Planning_ID }, { $set: NotNullObject }
      );
    ...
    res.status(200).json({
      success: `${PostsArray.length} Projects Updated Successfully!`
    });
  }
