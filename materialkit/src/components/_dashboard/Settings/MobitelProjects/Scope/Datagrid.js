...
export default function Datagrid() {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/Scope`);
  ...
  const deleteUser = async (selectedIDs) => {
    await axiosInstance.delete(`/Scope/delete/${selectedIDs}`).catch((err) => {
  ...
  const handleEditRowsModelChange = React.useCallback(
    (model) => {
      ...
      if (editedIds.length === 0) {
        ...
        if (confirmBox === true) {
          axiosInstance.put(`/Scope/Edit/${editRowData._id.value}`, newPost);
