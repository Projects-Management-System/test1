...
export default function SpecialTagDatagrid() {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/specialTag`);
    setState(res.data.existingTags);
  };
        ...
        <IconButton
          ...
          onClick={() => {
            ...
            if (confirmBox === true) {
              ...
              alert('Special Tag Deleted successfully!');
              fetchData();
  ...
  const deleteUser = async (selectedIDs) => {
    await axiosInstance.delete(`/specialTag/delete/${selectedIDs}`).catch((err) => {
  ...
  const handleEditRowsModelChange = React.useCallback(
    (model) => {
      ...
      if (editedIds.length === 0) {
        ...
        const newPost = { Special_Tag };
        const confirmBox = window.confirm('Do you want to save this name ?');
        if (confirmBox === true) {
          axiosInstance.put(`/specialTag/Edit/${editRowData._id.value}`, newPost);
