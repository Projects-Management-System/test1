...
export default function Datagrid() {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/Sub_Contractor`);
  ...
  const deleteUser = async (selectedIDs) => {
    await axiosInstance.delete(`/Sub_Contractor/delete/${selectedIDs}`).catch((err) => {
  ...
  const handleEditRowsModelChange = React.useCallback(
    (model) => {
      ...
      if (editedIds.length === 0) {
        ...
        const newPost = { Sub_Contractor };
        const confirmBox = window.confirm('Do you want to save this name ?');
        if (confirmBox === true) {
          axiosInstance.put(`/Sub_Contractor/Edit/${editRowData._id.value}`, newPost);
