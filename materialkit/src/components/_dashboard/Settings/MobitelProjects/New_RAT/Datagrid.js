...
export default function Datagrid() {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/New_RAT`);
        ...
        <IconButton
          ...
          onClick={() => {
            ...
            if (confirmBox === true) {
              ...
              alert('New RAT Deleted successfully!');
              fetchData();
  ...
  const deleteUser = async (selectedIDs) => {
    await axiosInstance.delete(`/New_RAT/delete/${selectedIDs}`).catch((err) => {
  ...
  const handleEditRowsModelChange = React.useCallback(
    (model) => {
      ...
      if (editedIds.length === 0) {
        ...
        const newPost = { New_RAT };
        const confirmBox = window.confirm('Do you want to save this name ?');
        if (confirmBox === true) {
          axiosInstance.put(`/New_RAT/Edit/${editRowData._id.value}`, newPost);
