...
export default function DependencyDatagrid() {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/Dependency`);
  ...
  const deleteUser = async (selectedIDs) => {
    await axiosInstance.delete(`/Dependency/delete/${selectedIDs}`).catch((err) => {
  ...
  const handleEditRowsModelChange = React.useCallback(
    (model) => {
      ...
      if (editedIds.length === 0) {
        ...
        const newPost = { Dependency };
        const confirmBox = window.confirm('Do you want to save this name ?');
        if (confirmBox === true) {
          axiosInstance.put(`/Dependency/Edit/${editRowData._id.value}`, newPost);
