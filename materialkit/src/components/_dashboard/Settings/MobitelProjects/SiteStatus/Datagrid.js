...
export default function Datagrid() {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/SiteStatus`);
        ...
        <IconButton
          ...
          onClick={() => {
            ...
            if (confirmBox === true) {
              ...
              alert('Site Status Deleted successfully!');
              fetchData();
  ...
  const deleteUser = async (selectedIDs) => {
    await axiosInstance.delete(`/SiteStatus/delete/${selectedIDs}`).catch((err) => {
  ...
  const handleEditRowsModelChange = React.useCallback(
    (model) => {
      ...
      if (editedIds.length === 0) {
        ...
        const newPost = { Site_Status };
        const confirmBox = window.confirm('Do you want to save this name ?');
        if (confirmBox === true) {
          axiosInstance.put(`/SiteStatus/Edit/${editRowData._id.value}`, newPost);
