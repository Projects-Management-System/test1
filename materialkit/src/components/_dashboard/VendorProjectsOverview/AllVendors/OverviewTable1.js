...
export default function OverviewTable1() {
  ...
  const getOverviewData = async () => {
    const result = await axiosInstance.get('/AutomateVendorProjectsOverviewTable');
    setoverviewData(result.data.OverviewDataForFrontendOverviewTable);
  };

  const getProjectNamesArray = async () => {
    const result = await axiosInstance.get('/vendorProjectsOverviewTableProjectsArray');
    setProjectNames(result.data.vendorProjectsNamesArrayToTheExcelUploads);
  };
